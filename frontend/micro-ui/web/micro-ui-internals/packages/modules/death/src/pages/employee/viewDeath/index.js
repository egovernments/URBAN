...
const ViewDeath = () => {
  ...
  useEffect(() => {
    if (certificateViewData?.DeathCertificate && certificateViewData.DeathCertificate.length > 0) {
      ...
      if (Hospitalname) {
        // Only add if Hospitalname is present
  ...
  const handlePayAndDownload = async () => {
    ...
    const businessService = "DEATH_CERT";
    history.push(`/digit-ui/employee/payment/collect/${businessService}/${fallbackConsumerCode}/tenantId=${tenantId}?workflow=death`);
    ...
    if (finalConsumerCode !== fallbackConsumerCode) {
      history.replace(`/digit-ui/employee/payment/collect/${businessService}/${finalConsumerCode}/tenantId=${tenantId}?workflow=death`);
    }
  };
