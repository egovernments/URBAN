...
const PTSelectRoadFactor = ({ t, config, onSelect, userType, formData, setError, clearErrors, formState }) => {
  ...
  if (userType === "employee") {
    return (
      <div>
        <LabelFieldPair>
          <CardLabel className="card-label-smaller">{t("Road Factor") + " *"}</CardLabel>
          <Controller
            name="locality"
            defaultValue={null}
            control={control}
...
export default PTSelectRoadFactor;
