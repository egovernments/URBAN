...
//       const translated = t(`ACTION_TEST_${index.toUpperCase().replace(/[ -]/g, "_")}`)?.toLowerCase();
//       const matchesSearch = search === "" || translated.includes(search.toLowerCase());

//       if (item.path && matchesSearch) {
//         if (!configEmployeeSideBar[index]) {
//           configEmployeeSideBar[index] = [item];
//         } else {
//           configEmployeeSideBar[index].push(item);
