...
function createAmountFieldFormatter(values,prefix,suffix){
	...
	var makeBudgetDetailTable = function() {
		var budgetDetailColumns = [ 
			...
			<c:if test="%{shouldShowField('fund')}">				
				{key:"fund.id",label:'<!-- TODO: Manual migration required for custom Struts tag -->'+mandatorySign('fund'),width:130,formatter:createDropdownFormatter(BUDGETDETAILLIST),  dropdownOptions:fundOptions} ,
			</c:if>
			<c:if test="%{shouldShowField('function')}">				
				{key:"function.id",label:'<!-- TODO: Manual migration required for custom Struts tag -->'+mandatorySign('Function'),width:450,formatter:createDropdownFormatter(BUDGETDETAILLIST),  dropdownOptions:functionOptions} ,
			</c:if>
